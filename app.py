...
def processSounds(audioFilePath,allSounds,folderPath,readingFiles):
    ...
    while i<10:
        ...
        # allSoundsPath = os.path.join(allSoundsPath,readingFiles)

        # print('allSoundsPath: ', allSoundsPath)
...
def index():
    ...
    # return res.read() ## this has the email, the id and the picture
    access_token = session.get('access_token')
...
def savegoogleAccount():
    ...
    mycursor.execute("INSERT INTO filtervideo (userId) VALUES(%s)",str(userId))
...
def isFilteredYes():
    ...
    if request.method == 'GET':
        ...
        #print(mycursor.rowcount, "isFilter  successfully updated")
...
def cosineSimilarity(sortedFreq):
    ...
    while i<(noOfClusters-1):
        if maxNumber == dist_out[3][i]:
            maxNumberIndex = i
        i += 1
    #print('maxNumberIndex: ',maxNumberIndex)
...
def getClusterHeadsIds(): ## to get the  movie Ids of each cluster
    ...
    while clusterHead < noOfClusters:
        ...
        for x in myresult:
            ...
            # print('Objects of each cluster: ', x[1])
        clusterHead+=1
...
def constructAllObjectsInOrder(same):
    ...
    #print("movieClusterIds: ", movieClusterIds)
    #print("------------------------")
    ...
    #print("sortedFrequency: ", sortedFreq)
...
def readVideo(objectsDetected,noOfOccurences):
    ...
    #print("movieIdArray in read func: ",movieIdArray)
    same=False
    clusterId = 0
    i = 0
    while clusterId < noOfClusters:
        # sql = "SELECT movieUrl FROM cosinesimilaritytable INNER JOIN movie ON cosinesimilaritytable.secondMovieId = movie.ID AND cosinesimilaritytable.cosineSimilarityValue>0.5 AND cosinesimilaritytable.firstMovieId=1 "
        ...
        for x in myresult:
            # clusterIdCounter=0
            #  #print (x)
            # if clusterId==clusterIdCounter:
            #     movieClusterIds.append(x[1])
            #     if x[1] in movieClusterIds:
            #         continue
                # else
             ...
             # print('Objects of each clusterHead: ',x[2])
             # print(i)
...
def readForConfusionMatrix(arrayOfArrays, arrayOfArraysFrequency, NoOfOccurencesTotal, Total, same,
    ...
    # column = 0
    # while column < len(objectsDetected):
    #     lastRow = numberOfVideosTopN - 1
    #     arrayOfArrays[lastRow].\
    #         append(objectsDetected[column])
    #     arrayOfArraysFrequency[lastRow].append(noOfOccurences[column])
    #     column += 1
    arrayOfArrays.append(objectsDetected)
    arrayOfArraysFrequency.append(noOfOccurences)

    # swapMatrixRows(arrayOfArrays)
    # swapMatrixRows(arrayOfArraysFrequency)

    #print('arrayOfArraysFrequency in Top N: ', arrayOfArraysFrequency)
    #print('arrayOfArrays in Top N: ', arrayOfArrays)
    ...
    #     while True:
    #
    #         # counter1=len(arrayOfArrays[i])
    #         print('arrayOfArrays[i]: ',len(arrayOfArrays[i]))
    #         counter1=4
    #
    #         if counter1==0:
    #             break
    ...
    #             if counter2 == 0:
    #                 break
    #         if j < len(arrayOfArrays[i]):
    #             if arrayOfArrays[i][j] ==Total[k]:
    #
    #                     sql = "INSERT INTO sortedfrequencies (MovieID,objectName,Frequency) values(%s,%s,%s)"
    #
    #                     val = (moviesCount, arrayOfArrays[i][j], arrayOfArraysFrequency[i][j])
    ...
    #                     sql = "INSERT INTO sortedfrequencies (MovieID,objectName,Frequency) values(%s,%s,%s)"
    #
    #                     val = (moviesCount, Total[k], 0)
    #                     mycursor.execute(sql, val)
    #                     db_connection.commit()
    #                     k += 1
    #                     ###
    #                 else:
    #                     arrayOfArrays[i][j] != Total[k]
    #                     sql = "INSERT INTO sortedfrequencies (MovieID,objectName,Frequency) values(%s,%s,%s)"
    #
    #                     val = (moviesCount, Total[k], 0)
    #                     mycursor.execute(sql, val)
    ...
    #     if moviesCount>mainResultValue:
    #         break
...
def getConfusionMatrix(topNVariableHere):
    ...
    #print("sorted frequency in Confusion matrix: ",sortedFreq)
...
def getRelatedVideosLinks():
    ...
    movieIdArray = getClusterHeadsIds()
    # print("movieIdArray in getRelatedVideosLinks",movieIdArray[0])
    # global maxNumberIndex
    # print("maxNumberIndex in getRelatedVideosLinks",maxNumberIndex," and movieIdArray[maxNumberIndex]: ",movieIdArray[maxNumberIndex])
...
def cosineSimilarityTopN(sortedFreq,movieIdArrayInOneCluster):
    dist_out = 1 - pairwise_distances(sortedFreq, metric="cosine")
    #print('dist_out in Top N: ',dist_out)
    ...
    while i<numberOfTopN:
        ...
        i+=1
    #print("max Number before: ",maxNumbers)
    # i=0
    # while i<len(movieIdArrayInOneCluster):
    #     maxNumbers.append(movieIdArrayInOneCluster[i])
    #     i+=1
    ...
    while topNIndex<numberOfTopN:
        ...
        while i<(numberOfMovies-1):
            ...
            i+=1
        #print('maxNumber in Top N no.',topNIndex,' is ',maxNumbers[topNIndex])
        # print('topNIndex: ',topNIndex)
        topNIndex+=1
    topNIndex = 0
    maxNumberIndices = []
    while topNIndex < numberOfTopN:
        #print("max numbers array after filled: ",maxNumbers)
        i=0
        #print("top N index is ",topNIndex)
        while i<(numberOfMovies-1):

            if maxNumbers[topNIndex] \
                    == dist_out[len(dist_out)-1][i]:
                maxNumberIndices.append(i)
                #print('maxNumberIndices in ',topNIndex,' = ', maxNumberIndices)
                topNIndex += 1
                if topNIndex>2:
                    break
            i += 1
    #print('maxNumberIndices in Top N: ',maxNumberIndices)
    topNIndex=0
    while topNIndex<numberOfTopN:
...
def getTopN(similarVideoClusterId): #Get top 3 videos similar to the unseen video
    ...
    sql2 = " ORDER BY movie.ID ASC"
    # mycursor.execute("SELECT * FROM movie WHERE clusterId=" + str(similarVideoClusterId))
...
def readTopN(arrayOfArrays,arrayOfArraysFrequency,NoOfOccurencesTotal,Total,same,numberOfVideosInCluster,idS):
    ...
    #print("sortedFrequency in Top N: ", sortedFreq)
...
maxNumberIndex = -1
# readTest()
objectsDetected=['microwave', 'oven', 'bowl', 'refrigerator', 'person', 'tvmonitor', 'mouse', 'cup', 'sandwich', 'carrot', 'spoon', 'pizza', 'bus', 'train', 'cell phone', 'clock', 'orange', 'bottle', 'fork', 'keyboard', 'wine glass', 'cake']
noOfOccurences=[1100, 2602, 1330, 14, 702, 60, 6, 41, 2, 217, 45, 182, 16, 1, 59, 128, 1, 21, 4, 3, 1, 3]
...
def success(filename):
    ...
    # # print (object)
    # print (obj)
    # print(noOfOccurences)
    # m = 0
    # for obj in objectsDetected:
    #     sql = "INSERT INTO processed (MovieID,Object,Frequency) values(%s,%s,%s)"
    #
    #     val = (4, obj, noOfOccurences[m])
    #     m += 1
    #     mycursor.execute(sql, val)
    #     db_connection.commit()
    ...
    #                        ,
    ## start=success.start_page,end=success.end_page, ## this was for testing don't uncomment it
                           # name=success.file_name
                           ## ,labels=LABELS          ## This was for testing don't uncomment it
                           # )
...
def getURL():
    ...
    if request.method == 'POST':
        ...
        ydl_opts = {}
        os.chdir('../pythonAnyWhere1/static/')
